...
procConstr (Abs.TConst (UIdent name) ts) =
  TConstr (Prefix name) (fmap procType ts)
procConstr (Abs.TConstEmpty (UIdent name)) = TConstr (Prefix name) []
procConstr (Abs.TInfixConst t (Abs.TIConstName (Abs.InfixFunctionNameF name)) ts)
  = TConstr (Infix name) (fmap procType (t : ts))
...
trimKName :: String -> FName
trimKName = Prefix . tail . init
...
procExp (Abs.EVar (Abs.FTCName (UIdent name))) = EVar (Prefix name)
...
procExp (Abs.EOpA e1 (Abs.InfixFunctionNameA name) e2) =
  EApp (EApp (EVar $ Infix name) (procExp e1)) (procExp e2)
...
procFBName (Abs.FBName (LIdent name)) = Prefix name
